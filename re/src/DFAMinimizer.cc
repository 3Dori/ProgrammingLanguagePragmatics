...
void DFAMinimizer::splitMergedDfaState(const MergedDfaState& node, const char sym) {
    std::map<int32_t, MergedDfaState*> newTransitions;
    const auto id = node.id;
    for (auto const* dfaState : node.dfaStates) {
        auto const* toNode = dfaState->m_transitions.at(sym);
        int32_t to = m_DFAToMergedDFA[toNode->m_id];

        if (newTransitions.find(to) == newTransitions.end()) {
            newTransitions[to] = makeMergedDfaState(node.isFinal);
        }
